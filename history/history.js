...
// --- UPDATED FUNCTION ---
// Renders the detailed HTML, now including the copy button.
function renderJobDetails(data) {
  if (!data) return '<p>No details available.</p>';

  // Use the task ID and job ID to create a more unique ID for the elements
  ...
  // --- NEW --- Add the copy button with a unique ID
  detailsHtml += `
    <h4>
    ...
    header.addEventListener('click', () => {
      ...
      if (isVisible) {
        ...
        if (!detailsContainer.innerHTML) {
          ...
          // --- NEW --- Find and attach the copy button listener AFTER rendering
